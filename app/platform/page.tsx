...
// ─── Persistence habitat encoding — 2026-04-27 LOCKED ────────────────────────

// Habitat centers — aspect-calibrated for deployed panel (~700×440px)
// VL bottom-left → VH upper-right structural gradient
...
// Seeded ticker hash — deterministic angle, stable across all renders
// No analytical meaning. Visual continuity only.
...
// Habitat radius — sqrt(bucket_share_current) scalar
// Gentler option: Moderate won't dominate, VL won't shrink dramatically
function habitatRadius(share: number, panelW: number): number {
  const BASE = panelW * 0.18  // base radius at equal share (20%)
    ...
    <section style={s({ position: 'relative', borderBottom: `1px solid ${E.bdr2}` })}>
      ...
      <div style={s({ position: 'relative', overflow: 'hidden' })}>
        ...
        <div style={s({ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 11 })}>
          ...
          <div style={s({ fontFamily: E.mono, fontSize: 11, color: E.sec })}>$249 / month · Cancel anytime</div>
        </div>
      </div>
    </section>
  )
  ...
  useEffect(() => {
    ...
    async function initViz() {
      ...
      try {
        ...
        // Fetch OSMR snapshot + persistence snapshot — join on symbol
        ...
        const persData: PersistenceRecord[] = persRes?.ok
          ? await persRes.json().catch(() => [])
          : []

        // Build persistence lookup by symbol
        const persMap = new Map<string, PersistenceRecord>(
          persData.map(p => [p.symbol, p])
        )
        ...
        if (!conEl) { console.error('[TCS] Structural Field SVG ref null'); return }

        const SW = conEl.clientWidth  || 700
        const SH = conEl.clientHeight || 440

        d3.select(conEl)
          .attr('width',  SW)
          .attr('height', SH)

        // EV bounds for node size encoding — 3rd/97th percentile
        ...
        // ── Persistence-radius + ticker-hash-angle placement ─────────────────
        // LOCKED 2026-04-27:
        //   Radius  → persistence_months / 24, settled = center, newly-migrated = periphery
        //   Angle   → seeded-stable ticker hash, no analytical meaning
        //   Opacity → bucket condition (BUCKET_OPACITY), NOT persistence
        //   Size    → enterprise value (nodeRadius)

        const PAD = 8  // 8px boundary clamp
        ...
        const cnGroups = d3.select(conEl).selectAll('.sn-wrap')
          ...
          .attr('transform',   (d: Node) => {
            const pers = persMap.get(d.symbol)
            const center = HABITAT_CENTERS[d.bucket]
            if (!center) return `translate(${SW / 2},${SH / 2})`

            // Habitat radius scaled by sqrt(bucket_share)
            const share = pers?.bucket_share_current ?? 0.20
            const hR = habitatRadius(share, SW)

            // Persistence radius: settled = center (0), newly-migrated = periphery (1)
            const persistenceT = pers ? Math.min(pers.persistence_months / 24, 1) : 0.5
            const radialFraction = 1 - persistenceT  // invert: high persistence → near center
            const radius = radialFraction * hR

            // Angle: seeded-stable ticker hash
            const angle = tickerHash(d.symbol) * 2 * Math.PI

            const cx = center.cx * SW + radius * Math.cos(angle)
            const cy = center.cy * SH + radius * Math.sin(angle)

            // Clamp to panel bounds
            const nodeR = nodeRadius(d.ev ?? evLo, evLo, evHi)
            const x = Math.max(PAD + nodeR, Math.min(SW - PAD - nodeR, cx))
            const y = Math.max(PAD + nodeR, Math.min(SH - PAD - nodeR, cy))

            return `translate(${x},${y})`
          })

        // Decorative habitat labels — limbus opacity, doctrinally M1
      ...
      <div style={s({ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: `1px solid ${E.bdr2}`, background: E.bg3 })}>
        <div style={s({ padding: '7px 18px', borderRight: `1px solid ${E.bdr2}` })}>
          ...
          <div style={s({ fontFamily: E.mono, fontSize: 11, color: E.sec, marginTop: 3 })}>
            The full U.S. equity universe, organized by structural condition · Neighbors share a condition, not a sector · Distance from center = months in that condition
          </div>
        </div>
      ...
      <div style={s({ position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr', height: 440, borderBottom: `1px solid ${E.bdr2}` })}>
        ...
        {!vizReady && (
          <div style={s({ position: 'absolute', inset: 0, background: E.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 5 })}>
            ...
            <div style={s({ fontFamily: E.mono, fontSize: 11, color: E.sec, letterSpacing: '0.14em' })}>Mapping structural habitats · ~5,200 equities</div>
            <div style={s({ fontFamily: E.mono, fontSize: 11, color: E.sec, marginTop: 5, letterSpacing: '0.1em' })}>Loading persistence data</div>
          </div>
        )}
      </div>
