...
// Palette: P3 inverse dark · AAA verified on #181E19
// Auth: Clerk — unauthenticated → blurred preview + modal
//             → authenticated → live dashboard (no redirect, no bridge)
// Data: /public/data/*.json (static, no API routes)
// Tier: free = signed in, subscription !== 'active'
//       paid = subscription === 'active'
// /platform-beta is deprecated — dashboard now lives at /platform directly
...
function returnHeatText(v: number | null, sup: boolean) {
  ...
  if (v <= -0.12) return "#FFB8A0"
  if (v <= -0.03) return "#EC9D6F"
  if (v <   0.03) return E.body
  if (v <   0.12) return "#7CC89C"
  return "#70BE8B"
}
...
const CLERK_APPEARANCE = {
  variables: {
    colorBackground: '#1F2820',
    colorPrimary: '#C6A24A',
    colorText: '#F0F4EF',
    colorTextSecondary: '#B8C4B9',
    colorTextOnPrimaryBackground: '#181E19',
    colorInputBackground: '#181E19',
    colorInputText: '#F0F4EF',
    colorNeutral: '#F0F4EF',
    borderRadius: '0px',
  ...
  elements: {
    ...
    socialButtonsBlockButton: 'border-[#2E3B30] rounded-none',
    footerActionLink: 'underline',
    footer: 'bg-transparent',
  },
}

// ─── Platform background (blurred auth preview) ────────────────────────────────
...
function PlatformBackground({ isPaid }: { isPaid: boolean }) {
  return (
    <div style={s({ width:"100%", height:"100%", overflow:"hidden", userSelect:"none", pointerEvents:"none" })}>
      ...
      <div style={s({ height:48, borderBottom:`1px solid ${E.bdr}`, display:"flex", alignItems:"center",
        padding:"0 28px", background:E.bg, justifyContent:"space-between" })}>
        <div style={s({ display:"flex", alignItems:"baseline" })}>
          <span style={s({ fontFamily:E.mono, fontSize:9, letterSpacing:"0.32em", textTransform:"uppercase" as const, color:E.muted })}>The Capital</span>
        ...
        <div style={s({ display:"flex", gap:16 })}>
          {["Market Map","Snapshot","Anchor Structure","History"].map(t => (
            <span key={t} style={s({ fontFamily:E.mono, fontSize:9, color:E.muted, letterSpacing:"0.1em" })}>{t}</span>
          ))}
        </div>
      </div>
      ...
      <div style={s({ display:"grid", gridTemplateColumns:"repeat(4,1fr)", borderBottom:`1px solid ${E.bdr}`, background:E.bg2 })}>
        {[
          { label:"Structural Stress",    val:"832"   },
          { label:"Improving Trajectory", val:"1,204" },
          { label:"Active Universe",      val:"5,168" },
      ...
      <div style={s({ display:"grid", gridTemplateColumns:"1fr 300px", height:"calc(100vh - 120px)" })}>
        <div style={s({ padding:"20px", background:E.bg })}>
          <div style={s({ fontFamily:E.mono, fontSize:8.5, letterSpacing:"0.18em", textTransform:"uppercase" as const, color:E.muted, marginBottom:14 })}>
            Structural Constellation · 5,168 companies
          </div>
          <div style={s({ position:"relative", height:340, border:`1px solid ${E.bdr}`, background:E.bg2 })}>
            {[
              {x:12,y:14,c:E.pos,sz:4},{x:24,y:22,c:E.pos,sz:5},{x:8,y:8,c:E.pos,sz:3},
              {x:36,y:40,c:E.gold,sz:4},{x:52,y:38,c:E.gold,sz:5},{x:44,y:52,c:E.gold,sz:3},
              {x:60,y:60,c:E.muted,sz:4},{x:72,y:68,c:E.muted,sz:5},{x:65,y:75,c:E.muted,sz:3},
              {x:80,y:78,c:E.negSoft,sz:4},{x:88,y:82,c:E.negSoft,sz:5},
              {x:92,y:88,c:E.neg,sz:5},{x:86,y:92,c:E.neg,sz:4},{x:94,y:94,c:E.neg,sz:6},
              {x:20,y:60,c:E.pos,sz:3},{x:30,y:72,c:E.gold,sz:4},{x:70,y:30,c:E.muted,sz:3},
              {x:50,y:20,c:E.pos,sz:4},{x:78,y:45,c:E.negSoft,sz:3},{x:15,y:85,c:E.gold,sz:4},
            ].map(({ x, y, c, sz }, i) => (
              <div key={i} style={s({ position:"absolute", left:`${x}%`, top:`${y}%`,
                width:sz*2, height:sz*2, borderRadius:"50%", background:c, opacity:0.7 })} />
            ))}
          </div>
        </div>
        <div style={s({ borderLeft:`1px solid ${E.bdr}`, background:E.bg2, padding:"18px" })}>
          <div style={s({ fontFamily:E.mono, fontSize:8.5, letterSpacing:"0.18em", textTransform:"uppercase" as const, color:E.muted, marginBottom:14 })}>
            Active Risk Cluster
          </div>
          {["NVDA","TSLA","AMZN","META","SNOW"].map((ticker, i) => (
            <div key={ticker} style={s({ display:"flex", justifyContent:"space-between", alignItems:"center",
              padding:"8px 0", borderBottom:`1px solid rgba(255,255,255,0.03)` })}>
              <span style={s({ fontFamily:E.mono, fontSize:12, color:E.text })}>{ticker}</span>
              <span style={s({ fontFamily:E.mono, fontSize:10, color: i<2 ? E.neg : i<4 ? E.negSoft : E.muted })}>
                {["Very High","Very High","High","High","Moderate"][i]}
              </span>
            </div>
          ))}
          <div style={s({ marginTop:18, padding:"12px", background:E.bg3, border:`1px solid ${E.bdr}` })}>
            <div style={s({ fontFamily:E.mono, fontSize:8.5, color:E.muted, marginBottom:8 })}>COMPANY DRILLDOWN</div>
            <div style={s({ fontFamily:E.sans, fontSize:13, fontWeight:700, color:E.text, marginBottom:6 })}>Snowflake Inc.</div>
            <div style={s({ display:"flex", flexDirection:"column", gap:4 })}>
              {["Anchor Detachment Risk","Anchor Degradation Risk","Composite"].map(l => (
...
function AuthModal() {
  ...
  return (
    <div style={s({ position:"fixed", inset:0, zIndex:50, display:"flex", alignItems:"center", justifyContent:"center" })}>
      <div style={s({ position:"absolute", inset:0, background:"rgba(14,16,15,0.75)", backdropFilter:"blur(10px)" })} />
      <div style={s({ position:"relative", width:"100%", maxWidth:480, background:E.bg2,
        border:`1px solid ${E.bdr2}`, borderTop:`2px solid ${E.gold}` })}>
        <div style={s({ padding:"28px 32px 20px" })}>
          <div style={s({ display:"flex", alignItems:"baseline", marginBottom:20 })}>
          ...
          {mode === 'prompt' && <>
            ...
            <div style={s({ display:"grid", gridTemplateColumns:"1fr 1fr", gap:4, marginBottom:24 })}>
              {[
                ...
                <div key={tier} style={s({ border:`1px solid ${primary ? E.bdr2 : E.bdr}`,
                  background: primary ? E.gatm : "transparent", padding:"16px" })}>
                  <div style={s({ fontFamily:E.mono, fontSize:8.5, letterSpacing:"0.16em", textTransform:"uppercase" as const,
                    color: primary ? E.gold : E.muted, marginBottom:12 })}>{tier}</div>
                  ...
                  <button onClick={action} style={s({ width:"100%", fontFamily:E.sans, fontSize:10.5, fontWeight:700,
                    letterSpacing:"0.08em", textTransform:"uppercase" as const, padding:"10px 0",
                    background: primary ? E.gold : "transparent",
                    color: primary ? E.bg : E.muted,
                    border: primary ? "none" : `1px solid ${E.bdr}`, cursor:"pointer" })}>
                    {cta}
                  </button>
                </div>
              ))}
            ...
            <div style={s({ textAlign:"center" as const })}>
              <button onClick={() => setMode('signin')} style={s({ fontFamily:E.mono, fontSize:9.5, color:E.muted,
          ...
          {mode === 'signin' && <>
            ...
            <div style={s({ textAlign:"center" as const, marginTop:16 })}>
              <button onClick={() => setMode('prompt')} style={s({ fontFamily:E.mono, fontSize:9.5, color:E.muted,
                background:"transparent", border:"none", cursor:"pointer", textDecoration:"underline" })}>← Back</button>
            </div>
          </>}
...
function TemporalBar({ manifest }: { manifest: HistoryManifestRow[] }) {
  ...
  return (
    <div style={s({ display:"flex", flexWrap:"wrap", gap:"6px 20px", padding:"7px 28px",
          ...
          <div key={label} style={s({ padding:"16px 22px", borderRight: i<3 ? `1px solid ${E.bdr}` : "none" })}>
            ...
            {locked
              ? <div style={s({ display:"flex", alignItems:"center", gap:7, marginBottom:4 })}>
                  <Lock size={13} style={{color:E.muted}}/>
                  <span style={s({ fontFamily:E.mono, fontSize:10, color:E.muted })}>Paid subscribers</span>
                </div>
...
function ConstellationMap({ data, onSelect, selectedSymbol }: {
  ...
  useEffect(() => {
    ...
    let t = 0; const dpr = window.devicePixelRatio || 1
    ...
    <div ref={containerRef} style={{ position:"relative", width:"100%", userSelect:"none" }}>
      ...
      <svg viewBox={`0 0 ${dims.w} ${dims.h}`} width={dims.w} height={dims.h}
        ...
        <g transform={`translate(${PAD.l+pw-4}, ${PAD.t+ph-22})`}>
          <circle cx={-140} cy={0} r={4} fill={E.gold} opacity={0.9}/>
          <circle cx={-140} cy={0} r={10} fill="none" stroke={E.gold} strokeWidth={1} opacity={0.3}/>
          <text x={-130} y={4} fontFamily={E.mono} fontSize={9} fill={E.muted}>
...
function MapTooltip({ row, x, y, containerRef }: {
  ...
  return (
    <div ref={ref} style={{ position:"absolute", left:pos.x, top:pos.y, background:E.bg,
      ...
      {[
        ["OAL",        row.oal_label ?? "—"],
        ["Detachment", fmtPct(row.axis1_pct)+"th pct"],
        ["Degradation",fmtPct(row.axis2_pct)+"th pct"],
        ...
        {top.map(row => {
          ...
          return (
            <div key={row.symbol} onClick={() => onSelect(row)} style={s({
              ...
              border:`1px solid ${isVH?"rgba(204,102,82,0.4)":E.bdr}`,
              background: isVH?"rgba(204,102,82,0.07)":"transparent",
            })}>
...
function Drilldown({ company, grid, isPaid, onClose }: {
  ...
  const axes = [
    {
      label:"Anchor Detachment Risk", bucket:a1, pct:company.axis1_pct,
    ...
    {
      label:"Anchor Degradation Risk", bucket:a2, pct:company.axis2_pct,
      ...
      {isPaid ? (
        <div style={s({ padding:"14px 16px", borderBottom:`1px solid ${E.bdr}` })}>
          ...
          <p style={s({ fontFamily:E.mono, fontSize:9.5, color:E.dim, marginBottom:10, lineHeight:1.5 })}>
            Detachment {a1} · Degradation {a2} · 12-month forward horizon
          </p>
        ...
        <table style={s({ width:"100%", borderCollapse:"collapse", minWidth:500 })}>
          <thead>
            <tr style={s({ background:E.bg2, borderBottom:`1px solid ${E.bdr}` })}>
              {["Symbol","OAL","Detachment Risk","Degradation Risk","Composite"].map(h => (
          ...
          <table style={s({ width:"100%", borderCollapse:"collapse" })}>
            <thead>
              <tr style={s({ borderBottom:`1px solid ${E.bdr}` })}>
                {["Anchor","Count","Detachment","Composite"].map(h => (
                  <th key={h} style={s({ fontFamily:E.mono, fontSize:9, letterSpacing:"0.14em",
...
export default function PlatformPage() {
  ...
  // Data state — fetched immediately, independent of auth resolution
              ...
              <div style={s({ padding:"20px 24px", borderBottom:`1px solid ${E.bdr}` })}>
                <div style={s({ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:6 })}>
                  ...
                  <span style={s({ fontFamily:E.mono, fontSize:9, color:E.dim })}>
                    X = Degradation Risk · Y = Detachment Risk · Color = Composite bucket
                  </span>
                </div>
                ...
                <ConstellationMap data={filtered} onSelect={setSelected} selectedSymbol={selected?.symbol}/>
              </div>
              ...
              {cohortGrid && (
                <div style={s({ padding:"20px 24px", borderBottom:`1px solid ${E.bdr}` })}>
                  ...
                  <p style={s({ fontFamily:E.sans, fontSize:12, color:E.body, marginBottom:14, lineHeight:1.65 })}>
                    How companies in each structural zone have historically performed. The bottom-right cells — high detachment, high degradation — carry the strongest signal.
                  </p>
                  {isPaid ? (
